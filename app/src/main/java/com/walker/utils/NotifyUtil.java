...
public class NotifyUtil {
    ...
    public static void playEffect(Context context) {
        //夜间模式，不处理
        if (NotifyUtil.isNightTime() && SPHelper.getBoolean(HbaoConfig.KEY_NIGHT_NOT, false)) {
            return;
        }

        if (SPHelper.getBoolean(HbaoConfig.KEY_NOTIFY_SOUND, false)) {
            sound(context);
        }
        if (SPHelper.getBoolean(HbaoConfig.KEY_NOTIFY_SHAKE, false)) {
            vibrator(context);
        }
    }
